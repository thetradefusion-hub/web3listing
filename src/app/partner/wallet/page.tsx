  ...
  return (
    <PartnerPageShell>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <PartnerStatCard title="Available Balance" value={formatCurrency(wallet?.available_balance || 0)} subtitle="Ready to withdraw" icon={Wallet} color="teal" />
        <PartnerStatCard title="Pending Balance" value={formatCurrency(wallet?.pending_balance || 0)} subtitle="Processing" icon={Clock} color="orange" />
        <PartnerStatCard title="Lifetime Earnings" value={formatCurrency(wallet?.lifetime_earnings || 0)} subtitle="All time" icon={TrendingUp} color="purple" />
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <PartnerPanel>
          <PartnerPanelHeader title="Request Withdrawal" description="Withdraw your available commission balance" />
          <PartnerPanelBody>
        ...
        <PartnerPanel>
          <PartnerPanelHeader title="Withdrawal History" />
          <PartnerPanelBody className="space-y-2">
      ...
      <PartnerPanel>
        <PartnerPanelHeader title="Commission Ledger" description="Recent credits and debits" />
        <PartnerPanelBody className="space-y-2">
          {ledger && ledger.length > 0 ? (
            ...
            <PartnerEmptyState title="No ledger entries yet" description="Commissions will appear here after completed orders." />
          )}
        </PartnerPanelBody>
      </PartnerPanel>
    </PartnerPageShell>
