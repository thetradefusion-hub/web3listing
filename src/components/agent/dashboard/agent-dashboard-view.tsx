...
} from "@/components/agent/dashboard/ui";
...
import {
  ...
  ProjectTokenChip,
  SupportActionCard,
} from "@/components/agent/dashboard/dashboard-premium";
import { rel, AgentPageShell, AgentPanel, AgentPrimaryButton, AgentSecondaryButton } from "@/components/agent/ui";
...
export function AgentDashboardView({
  ...
  return (
    <AgentPageShell compact className="space-y-3">
      {kycRequired && (
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6 lg:gap-2.5">
        <AgentStatCard title="Total Projects" value={stats.projectCount} icon={FolderKanban} color="blue" />
        <AgentStatCard title="Total Orders" value={stats.orderCount} icon={ClipboardList} color="green" />
        <AgentStatCard title="Active Orders" value={stats.activeOrders} icon={Clock} color="orange" trend={stats.activeOrderPct} trendDirection="neutral" />
        <AgentStatCard title="Completed" value={stats.completedOrders} icon={CheckCircle2} color="purple" trend={stats.completionRate} />
        <AgentStatCard title="Total Earnings" value={formatCurrency(stats.lifetimeEarnings)} icon={DollarSign} color="teal" />
        <AgentStatCard
      ...
      <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-12 lg:items-stretch lg:gap-3">
        <AgentPanel className="premium-card-interactive flex h-full flex-col overflow-hidden lg:col-span-8">
          <PremiumPanelHeader
            title="Recent Orders"
        ...
        <AgentPanel className="premium-card-interactive flex h-full flex-col lg:col-span-4">
      ...
      <div className="grid grid-cols-1 gap-2.5 md:grid-cols-3 lg:gap-3">
        <AgentPanel className="premium-card-interactive overflow-hidden">
      ...
      <div className="grid grid-cols-1 gap-2.5 md:grid-cols-3 lg:gap-3">
        ...
        >
          {manager?.telegram_link ? (
            ...
            <AgentPrimaryButton href="/agent/support" className="mt-2.5 h-8 w-full text-xs">
              Open Support
            </AgentPrimaryButton>
          )}
        </SupportActionCard>
        ...
        >
          {manager ? (
            <div className="mt-2.5">
              ...
              {manager.telegram_link && (
                <AgentSecondaryButton href={manager.telegram_link} className="mt-2 h-8 w-full text-xs">
                  <Send className="h-3.5 w-3.5" />
                  Message on Telegram
