...
import { OrderReviewForm } from "@/components/agent/orders/order-review-form";
            ...
            {relatedServices.map((s) => (
              ...
              >
                ...
                <Button size="sm" className="h-7 shrink-0 rounded-lg bg-[#635BFF] px-2.5 text-[10px]" asChild>
                  <Link href={`/agent/services/${s.slug}`}>Order</Link>
                </Button>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          <Button variant="outline" size="icon" className="h-9 w-9 shrink-0 rounded-lg" asChild>
            <Link href={`/agent/orders/${order.id}`} aria-label="Back to order">
          ...
          <div className="min-w-0">
            ...
            <nav className="mt-1 flex flex-wrap items-center gap-1 text-xs text-[#94A3B8] sm:text-sm">
              ...
              <Link href={`/agent/orders/${order.id}`} className="truncate hover:text-[#635BFF]">
                #{order.order_number}
              </Link>
              <ChevronRight className="h-3.5 w-3.5 shrink-0" />
          ...
          <div className="flex min-w-0 items-start gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              ...
              <Button variant="outline" size="sm" className="mt-2 h-8 rounded-lg text-xs" asChild>
                <Link href={`/agent/projects/${project.id}`}>View Project</Link>
              </Button>
            </div>
          </div>
