...
import { OrderDetailTimeline } from "@/components/agent/orders/order-detail-timeline";
...
export function AgentOrderDetailView({
  order,
  service,
  project,
  payment,
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <Button variant="outline" size="icon" className="h-9 w-9 shrink-0 rounded-lg" asChild>
            <Link href="/agent/orders" aria-label="Back to orders">
          ...
          <div>
            ...
            <nav className="mt-2 flex items-center gap-1 text-xs text-[#94A3B8]">
              <Link href="/agent/orders" className="hover:text-[#635BFF]">My Orders</Link>
        ...
        {showDelivery && (
          <Button
            ...
            asChild
          >
            <Link href={`/agent/orders/${order.id}/delivery`}>View Delivery</Link>
          </Button>
        )}
      </div>
      ...
      {showDelivery && (
        <div className="flex flex-col gap-3 rounded-2xl border border-[#A7F3D0] bg-gradient-to-r from-[#ECFDF5] to-[#F0FDF4] p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button size="sm" className="rounded-lg bg-[#059669] hover:bg-[#047857]" asChild>
            <Link href={`/agent/orders/${order.id}/delivery`}>Open Delivery Page</Link>
          </Button>
        </div>
      )}
      ...
      {deliverables.length > 0 && (
        <section className="rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm sm:p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            {showDelivery && (
              <Link
                href={`/agent/orders/${order.id}/delivery`}
