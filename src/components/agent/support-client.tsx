...
import { AgentPanel, AgentPanelBody, AgentPanelHeader, AgentBadge, AgentEmptyState } from "@/components/agent/ui";
  ...
  return (
    <div className="space-y-4">
      <AgentPanel className="border-violet-200 bg-gradient-to-br from-violet-50 to-blue-50">
        <AgentPanelBody className="flex flex-wrap items-center justify-between gap-4">
          <div>
      ...
      {showForm && (
        <AgentPanel>
          <AgentPanelHeader title="Create Support Ticket" />
          <AgentPanelBody>
          ...
          <AgentPanel key={ticket.id}>
            <AgentPanelBody>
              <div className="flex items-start justify-between gap-3">
                ...
                <AgentBadge variant={ticketVariant(ticket.status)}>{ticket.status.replace(/_/g, " ")}</AgentBadge>
              </div>
            </AgentPanelBody>
          </AgentPanel>
        ))
      ) : (
        !showForm && <AgentEmptyState title="No tickets yet" description="Create a ticket or contact us on Telegram." />
      )}
    </div>
  );
}
